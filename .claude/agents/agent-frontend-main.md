...
10. ⚠️ **运行验证（必须执行，不通过不算完成）**：
   ...
   - `npm run dev` → 无红色控制台报错
   - 打开 http://localhost:5173 → 页面正常渲染
   - 如果任何一步失败 → 修复 → 重新验证 → 直到通过

### 步骤4：记录与交接（⚠️ 三个追踪文件必须全部更新，缺一不可）

> 代码完成 ≠ 任务完成。以下三步全部做完才算任务完成。

10. ⚠️ **必须更新 agent-logs**（只追加新行）：每个组件的设计理由、拒绝了什么替代方案
11. ⚠️ **必须更新 handoff-queue.md**：每次交付登记
12. ⚠️ **必须更新 decisions-log.md**：每次技术决策立即追加
13. 更新看板
...
- ❌ 不要写代码不运行就标记Done — 必须 `npm run dev` 无报错后才算完成
