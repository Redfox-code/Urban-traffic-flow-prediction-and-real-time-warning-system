...
5. 如果任务有 BlockedBy 标记 → 检查阻塞源是否已Done
   ...
   - ⚠️ **每次检查依赖后，如果阻塞解除，必须在 handoff-queue.md 登记**
...
### 步骤3：执行任务
9. ⚠️ **执行前先写日志**：在agent-logs/agent-lead-log.md中记录 🎯任务开始
10. **每完成一个子步骤**立即追加日志（💭思考/📝产出），不要等全部完成才写
11. ⚠️ **每做一个技术决策，立即追加到 decisions-log.md**：背景、至少2个选项、决议、理由。不等任务结束。
12. 如果是**站会主持**：
...
8. 如果是**开发任务**：
   ...
   - ⚠️ **运行验证（必须执行，不通过不算完成）**：
     ...
     - `flask --app run.py run --port 5000` → 检查无 import 错误/启动异常
     - `curl http://localhost:5000/api/v1/sections` → 验证返回 200
     - 如果任何一步失败 → 修复代码 → 重新验证 → 直到通过
   - 绝不要跳过设计直接写代码

### 步骤4：记录与交接（⚠️ 三个追踪文件必须全部更新，缺一不可）

> 代码完成 ≠ 任务完成。以下三步全部做完才算任务完成。

9. ⚠️ **必须更新 agent-logs/agent-lead-log.md**（只追加新行，绝不覆盖）：记录做了什么、为什么这样做、遇到什么问题、拒绝了什么替代方案
10. ⚠️ **必须更新 handoff-queue.md**：每次交付物产出后立即登记——交付方→接收方、交付物路径、下游Agent现在可以开始做什么
11. ⚠️ **必须更新 decisions-log.md**：本次任务中如有技术决策（选型/架构/参数/算法），立即追加——背景、选项、决议、理由
12. 更新 `.claude/board/task-board.md`：任务状态变更

## 禁止行为
- ❌ 不要跳过日志 — 每个任务必须记录 🎯开始→💭思考→📝产出→✅完成
- ❌ 不要删除日志 — agent-logs是只追加的，绝不覆盖或删除已有内容
- ❌ 不要写敷衍日志 — 思考轨迹必须具体到「为什么这样做」「有什么风险」「拒绝了什么替代方案」

## ⚠️ 追踪文件强制更新规则（违反即为任务未完成）

以下三个文件必须在每次任务完成后立即更新，**不允许攒到阶段结束才补**：

1. **agent-logs/agent-lead-log.md** — 每个任务必须追加 🎯→💭→📝→✅ 完整链
2. **.claude/board/handoff-queue.md** — 每次交付物产出后，立即登记：交付方→接收方、交付物路径、下游可以开始做什么
3. **.claude/board/decisions-log.md** — 每次技术决策立即追加
4. **run-log.md** — 每次任务完成/阶段里程碑追加 `[时间] 🤖Agent | ✅成功 | 任务ID+摘要`

> 如果任务完成了代码但没更新这四个文件，任务不算完成。
- ❌ 不要跳过设计直接写代码 — 先写设计文档再动手
- ❌ 不要写代码不运行就标记Done — 必须实际运行验证通过（flask run + curl）后才算完成
